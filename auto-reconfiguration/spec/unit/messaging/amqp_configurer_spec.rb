...
describe 'AutoReconfiguration::Rabbit' do
  ...
  it 'auto-configures AMQP on connect with url and new format (srs)' do
    conn_error = attempt_connect "amqp://testuser:testpass@127.0.0.1:1234/testvhost"
    conn_error.should =~ /Could not estabilish TCP connection to 10.20.30.40:12345/
  end
  ...
  it 'does not auto-configure Rabbit if VCAP_SERVICES not set' do
  ...
  it 'disables Rabbit auto-reconfig if DISABLE_AUTO_CONFIG includes rabbitmq' do
...
private
def attempt_connect(connection_options_or_string)
  conn_error = ''
  begin
    EventMachine.run do
